<script lang="ts">
	...
	export let onSelectMetric: (metric: OverviewMetric) => void = () => {};
...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
	{#each metrics as metric, index}
		...
		{@const isActionable = metric.label === "Webhook Gagal"}
		{#if isActionable}
			<button
				type="button"
				class={`animate-fade-in-up delay-${Math.min(index + 1, 6)} rounded-[20px] border border-stone-200/60 bg-white/70 p-5 text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 dark:border-white/10 dark:bg-white/5 dark:hover:shadow-black/20`}
			...
			>
				...
				<div class="mt-3 text-[12px] font-semibold text-orange-700 dark:text-orange-400">
					Buka tab Webhook
				</div>
			</button>
		{:else}
			<div class={`animate-fade-in-up delay-${Math.min(index + 1, 6)} rounded-[20px] border border-stone-200/60 bg-white/70 p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5 dark:border-white/10 dark:bg-white/5 dark:hover:shadow-black/20`}>
				<div class="mb-3 flex items-center justify-between">
					<span class="text-[13px] font-semibold uppercase tracking-wider text-stone-500 dark:text-stone-400">{metric.label}</span>
					<div class={`flex h-9 w-9 items-center justify-center rounded-lg ${toneMap[metric.tone]}`}>
						<Icon class="size-[18px]" />
