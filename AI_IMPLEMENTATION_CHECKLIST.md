...
- [x] Kontrak lookup Store API production `GET /v1/transactions/{order_id}` sudah diperbaiki dan dideploy pada `2026-05-15` sampai commit `d5f0bc6`: response sekarang kembali memakai bentuk charge/detail merchant-facing (`transaction_id`, `amount`, `payment_method`, `payment_type`, dan instruksi Midtrans), bukan shape database internal yang membuat checkout LinkSnap gagal menampilkan detail pembayaran.
- [x] Follow-up production LinkSnap untuk order `LS-1778806110022-cfdbe91dba67` sudah tervalidasi pada `2026-05-15`: lookup PayGate mengembalikan HTTP `200`, `status=pending`, `payment_type=bank_transfer`, `payment_method=bni`, `amount=128000`, VA tersedia, dan delivery webhook PayGate yang sebelumnya `failed_permanently` berhasil di-resend lalu berubah menjadi `success` dengan response LinkSnap HTTP `200`.
